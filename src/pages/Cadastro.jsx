...
const Cadastro = ({ onLogin }) => {
  ...
  const handleRegistrationClick = async () => {
    ...
    if (!nameError && !emailError && !passwordError && !confirmPasswordError) {
      try {
        const user = await registerUserOnServer(name, email, password);
      ...
      <div className="flex flex-col items-center cover">
        ...
        >
          <div className="flex flex-col gap-2">
            ...
            <input
              id="name"
              className={`p-[10px] outline-0 rounded bg-inptcl ${nameError && "border-red-500"}`}
              type="text"
              value={name}
          ...
          <div className="flex flex-col gap-2">
            ...
            <input
              id="confirmPassword"
              className={`p-[10px] outline-0 rounded bg-inptcl ${confirmPasswordError && "border-red-500"}`}
              type="password"
